// 乐队类型
export const TEAM_TYPE = [
	{ id: 1, name: '单人' },
	{ id: 2, name: '乐队(2-8人)' },
	{ id: 3, name: '乐团' },
];

// 演奏方式
export const PLAYS_STYLE = [
	{ id: 1, name: '器乐' },
	{ id: 2, name: '人声' },
];

// 擅长风格
export const person_style = [
	{ id: 1, name: '古典' },
	{ id: 2, name: '电子' },
	{ id: 3, name: '流行' },
	{ id: 4, name: '国风' },
	{ id: 5, name: '摇滚' },
	{ id: 6, name: '爵士' },
	{ id: 7, name: '布鲁斯' },
	{ id: 8, name: '独立音乐' },
	{ id: 9, name: '阿卡贝拉' },
];

// 器乐类型
export const instruments = [
	{ id: 1, name: '西洋·木管乐', url: '/instruments/xiyang_muguanyue.jpg' },
	{ id: 2, name: '西洋·铜管乐', url: '/instruments/xiyang_tongguanyue.jpg' },
	{ id: 3, name: '西洋·弦乐', url: '/instruments/xiyang_xuanyue.jpg' },
	{ id: 4, name: '西洋·键盘乐', url: '/instruments/xiyang_jianpanyue.jpg' },
	{ id: 5, name: '西洋·弹拨乐', url: '/instruments/xiyang_tanboyue.jpg' },
	{ id: 6, name: '西洋·打击乐', url: '/instruments/xiyang_dajiyue.jpg' },
	{ id: 7, name: '民乐·弹拨乐器', url: '/instruments/minyue_tanboyue.jpg' },
	{ id: 8, name: '民乐·吹管乐器', url: '/instruments/minyue_chuiguan.jpg' },
	{ id: 9, name: '民乐·拉弦乐器', url: '/instruments/minyue_laxuanyue.jpg' },
	{ id: 10, name: '民乐·打击乐器', url: '/instruments/minyue_dajiyue.jpg' },
	{ id: 10, name: '其他', url: '/instruments/bg_defalut.png' },
];

// 人声类型
export const voices = [
	{ id: 61, name: '民族唱法', url: '/instruments/rensheng_minzu.jpg' },
	{ id: 62, name: '美声唱法', url: '/instruments/rensheng_meisheng.jpg' },
	{ id: 63, name: '流行唱法', url: '/instruments/rensheng_liuxing.jpg' },
	{ id: 64, name: '通俗唱法', url: '/instruments/rensheng_liuxing.jpg' },
	{ id: 65, name: '合唱', url: '/instruments/rensheng_hechang.jpg' },
];

// 技能名称
export const skills = [
	{ id: 1, name: '小提琴' },
	{ id: 2, name: '钢琴' },
	{ id: 3, name: '电子琴' },
	{ id: 4, name: '吉他' },
];

// 学校
export const schools = [
	{
		id: 1,
		name: '中央音乐学院',
	},
	{
		id: 3,
		name: '北京舞蹈学院',
	},
	{
		id: 4,
		name: '中国音乐家协会',
	},
	{
		id: 5,
		name: '中国民族管弦乐学会',
	},
	{
		id: 6,
		name: '中国艺术科技研究所',
	},
	{
		id: 7,
		name: '中国艺术职业教育学会',
	},
	{
		id: 8,
		name: '中国歌剧舞剧院',
	},
	{
		id: 9,
		name: '中国舞蹈家协会',
	},
	{
		id: 10,
		name: '教育部考试中心',
	},
	{
		id: 11,
		name: '中国书法家协会',
	},
	{
		id: 12,
		name: '文化和旅游部艺术发展中心(原文化部艺术服务中心)',
	},
	{
		id: 14,
		name: '中华文化促进会',
	},
	{
		id: 15,
		name: '故宫博物院',
	},
	{
		id: 16,
		name: '国家京剧院',
	},
	{
		id: 17,
		name: '中国少年儿童发展服务中心',
	},
	{
		id: 18,
		name: '北京迷笛音乐学校',
	},
	{
		id: 19,
		name: '中央芭蕾舞团',
	},
	{
		id: 20,
		name: '北京爱乐吉他室内乐团',
	},
	{
		id: 21,
		name: '中国社会艺术协会',
	},
	{
		id: 22,
		name: '中国电视艺术家协会',
	},
	{
		id: 23,
		name: '文化和旅游部人才中心',
	},
	{
		id: 24,
		name: '中国文化管理协会',
	},
	{
		id: 25,
		name: '北京市教育学会',
	},
	{
		id: 26,
		name: '天津音乐学院',
	},
	{
		id: 27,
		name: '沈阳音乐学院',
	},
	{
		id: 28,
		name: '辽宁省舞蹈家协会',
	},
	{
		id: 29,
		name: '辽宁省群众艺术馆',
	},
	{
		id: 30,
		name: '吉林省舞蹈家协会',
	},
	{
		id: 31,
		name: '吉林省艺术学院',
	},
	{
		id: 32,
		name: '东北师范大学',
	},
	{
		id: 33,
		name: '吉林省文化馆',
	},
	{
		id: 34,
		name: '上海江南丝竹协会',
	},
	{
		id: 35,
		name: '上海中华书法协会',
	},
	{
		id: 36,
		name: '上海音乐家协会',
	},
	{
		id: 37,
		name: '上海语言文字工作者协会',
	},
	{
		id: 38,
		name: '上海美术家协会',
	},
	{
		id: 39,
		name: '上海书法家协会',
	},
	{
		id: 40,
		name: '上海音乐学院',
	},
	{
		id: 41,
		name: '上海打击乐协会',
	},
	{
		id: 42,
		name: '上海舞蹈家协会',
	},
	{
		id: 43,
		name: '上海金口才朗诵演讲团',
	},
	{
		id: 44,
		name: '江苏省戏剧学校',
	},
	{
		id: 45,
		name: '江苏省社会艺术教育中心',
	},
	{
		id: 46,
		name: '江苏省音乐家协会',
	},
	{
		id: 47,
		name: '江苏省舞蹈家协会',
	},
	{
		id: 48,
		name: '江苏省文化艺术联合会',
	},
	{
		id: 49,
		name: '江苏人民艺术剧院',
	},
	{
		id: 50,
		name: '江苏省朗诵协会',
	},
	{
		id: 51,
		name: '南京艺术学院',
	},
	{
		id: 52,
		name: '苏州评弹学校',
	},
	{
		id: 53,
		name: '中国美术学院',
	},
	{
		id: 54,
		name: '浙江艺术职业学院',
	},
	{
		id: 55,
		name: '浙江歌舞剧院',
	},
	{
		id: 56,
		name: '浙江省美术家协会',
	},
	{
		id: 57,
		name: '浙江省电影家协会',
	},
	{
		id: 58,
		name: '浙江省戏剧家协会',
	},
	{
		id: 59,
		name: '浙江音乐家协会',
	},
	{
		id: 60,
		name: '浙江省舞蹈家协会',
	},
	{
		id: 61,
		name: '安徽省文化馆',
	},
	{
		id: 62,
		name: '安徽艺术职业学院',
	},
	{
		id: 63,
		name: '福建省管乐艺术协会',
	},
	{
		id: 64,
		name: '江西省美术家协会',
	},
	{
		id: 65,
		name: '江西省书法家协会',
	},
	{
		id: 66,
		name: '江西省舞蹈家协会',
	},
	{
		id: 67,
		name: '山东省舞蹈家协会',
	},
	{
		id: 68,
		name: '山东省文化馆',
	},
	{
		id: 69,
		name: '湖北艺术职业学院',
	},
	{
		id: 70,
		name: '湖北省群众艺术馆',
	},
	{
		id: 71,
		name: '武汉音乐学院',
	},
	{
		id: 72,
		name: '湖北省音乐家协会',
	},
	{
		id: 73,
		name: '湖南省舞蹈家协会',
	},
	{
		id: 74,
		name: '广东省音乐家协会',
	},
	{
		id: 75,
		name: '广东省文化馆',
	},
	{
		id: 76,
		name: '星海音乐学院',
	},
	{
		id: 77,
		name: '广西艺术学院',
	},
	{
		id: 78,
		name: '广西艺术学校',
	},
	{
		id: 79,
		name: '四川省文化馆',
	},
	{
		id: 80,
		name: '四川省音乐舞蹈研究所',
	},
	{
		id: 81,
		name: '四川省音乐家协会',
	},
	{
		id: 82,
		name: '四川省舞蹈家协会',
	},
	{
		id: 83,
		name: '四川省艺术研究院',
	},
	{
		id: 84,
		name: '四川音乐学院',
	},
	{
		id: 85,
		name: '四川艺术职业学院',
	},
	{
		id: 86,
		name: '云南艺术学院',
	},
	{
		id: 87,
		name: '云南大学',
	},
	{
		id: 88,
		name: '陕西省儿童文化艺术学会',
	},
	{
		id: 89,
		name: '西安美术学院',
	},
	{
		id: 90,
		name: '陕西省艺术馆',
	},
	{
		id: 91,
		name: '西安音乐学院',
	},
	{
		id: 92,
		name: '陕西省黑池文化艺术协会',
	},
	{
		id: 93,
		name: '新疆画院',
	},
	{
		id: 94,
		name: '新疆美术家协会',
	},
	{
		id: 95,
		name: '新疆音乐家协会',
	},
	{
		id: 96,
		name: '新疆舞蹈家协会',
	},
	{
		id: 97,
		name: '新疆艺术馆',
	},
];

// 证书等级
export const levels = [
	{ id: 1, name: '一级' },
	{ id: 2, name: '二级' },
	{ id: 3, name: '三级' },
	{ id: 4, name: '四级' },
	{ id: 5, name: '五级' },
	{ id: 6, name: '六级' },
	{ id: 7, name: '七级' },
	{ id: 8, name: '八级' },
	{ id: 9, name: '九级' },
	{ id: 10, name: '十级' },
	{ id: 11, name: '演奏级' },
];

// 学习形式
export const study_type = [
	{ id: 1, name: '全日制' },
	{ id: 2, name: '非全日制' },
	{ id: 3, name: '成人大学' },
];

// 参与报价的状态 1-未参与竞标 2-竞标进行中待商议 3-被拒绝  4-中标
export const price_state = [
	{ id: 1, name: '竞标中' },
	{ id: 2, name: '竞标中' },
	{ id: 3, name: '已拒绝' },
	{ id: 4, name: '同意报价' },
];

// 需求的状态 1-竞价进行中 2-竞价结束，需求进行中（未支付） 3-需求进行中（已支付） 4-需求取消  5-待付款给用户 6-交易成功 7-交易取消
export const DEMAND_STATE = [
	{ id: 1, name: '竞标中' },
	{ id: 2, name: '待支付' },
	{ id: 3, name: '进行中' },
	{ id: 4, name: '已取消' },
	{ id: 5, name: '待付款给演员' },
	{ id: 6, name: '已完成(待评价)' },
	{ id: 7, name: '已完成(已评价)' },
	{ id: 8, name: '已取消' },
];

// 演员看到的需求的状态 1-竞价进行中 2-竞价结束，需求进行中（未支付） 3-需求进行中（已支付） 4-需求取消  5-待付款给用户 6-交易成功 7-交易取消
export const DEMAND_STATE_FOR_ACTOR = [
	{ id: 1, name: '竞标中' },
	{ id: 2, name: '需求方待支付' },
	{ id: 3, name: '进行中' },
	{ id: 4, name: '已取消' },
	{ id: 5, name: '需求方待确认' },
	{ id: 6, name: '已完成(待评价)' },
	{ id: 7, name: '已完成(已评价)' },
	{ id: 8, name: '已取消' },
];

// 队员参与状态
export const TEAM_USER_STATE = [
	{ id: 1, name: '未加入' },
	{ id: 2, name: '' }, // 已同意
	{ id: 3, name: '已拒绝' },
];

// 乐队成员担当
export const TEAM_USER_SKILL = [
	{ id: 1, name: '钢琴手' },
	{ id: 2, name: '小提琴手' },
	{ id: 3, name: '架子鼓手' },
	{ id: 4, name: '贝斯手' },
	{ id: 5, name: '主唱' },
	{ id: 6, name: '手风琴手' },
];
